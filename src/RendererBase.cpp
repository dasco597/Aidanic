...
VkResult RendererBase::createDebugUtilsMessengerEXT(VkInstance instance, const VkDebugUtilsMessengerCreateInfoEXT* pCreateInfo, const VkAllocationCallbacks* pAllocator, VkDebugUtilsMessengerEXT* pDebugMessenger) {
...
void RendererBase::destroyDebugUtilsMessengerEXT(VkInstance instance, VkDebugUtilsMessengerEXT debugMessenger, const VkAllocationCallbacks* pAllocator) {
...
RendererBase::QueueFamilyIndices RendererBase::findQueueFamilies(VkPhysicalDevice device) {
...
bool RendererBase::checkDeviceExtensionSupport(VkPhysicalDevice device) {
...
RendererBase::SwapChainSupportDetails RendererBase::querySwapChainSupport(VkPhysicalDevice device) {
...
VkSurfaceFormatKHR RendererBase::chooseSwapSurfaceFormat(const std::vector<VkSurfaceFormatKHR>& availableFormats) {
...
VkPresentModeKHR RendererBase::chooseSwapPresentMode(const std::vector<VkPresentModeKHR>& availablePresentModes) {
...
VkExtent2D RendererBase::chooseSwapExtent(const VkSurfaceCapabilitiesKHR& capabilities) {
    if (capabilities.currentExtent.width != UINT32_MAX) {
        return capabilities.currentExtent;
    } else {
        int width, height;
        glfwGetFramebufferSize(ioInterface->GetWindow(), &width, &height);
...
VkShaderModule RendererBase::createShaderModule(const std::vector<char>& code) {
...
void RendererBase::createBuffer(VkDeviceSize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags properties, VkBuffer& buffer, VkDeviceMemory& bufferMemory) {
